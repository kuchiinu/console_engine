...
void DrawManager_init(void)
{
	...
	*/
	...
	SetConsoleScreenBufferSize(draw.handle, COORD{ 0,0 });
	SMALL_RECT m_rectWindow = { 0, 0, 1, 1 };
	SetConsoleWindowInfo(draw.handle, TRUE, &m_rectWindow);

	//バッファ画面のサイズ
	COORD coord = { (short)draw.screen_width, (short)draw.screen_height };//この二つの数字は画面の横幅と縦幅
...
//コンソールと描画機能を取得
DrawManager* GetActiveDrawManager(void)
{
	return &draw;
}